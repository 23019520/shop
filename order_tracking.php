...
<body>
    <div class="container">
        ...
        <div class="stats">
            ...
            <div class="stat-card">
    <h3>In Transit</h3>
    <div class="stat-value status-shipped"><?php echo $orderStats['in_transit']; ?></div>
</div>
...
<td>
    <?php if ($order['status'] === 'in_transit'): ?>
        <form action="location.php" method="get" style="display:inline;">
            <input type="hidden" name="order_id" value="<?php echo $order['history_id']; ?>">
            <button type="submit" style="padding:6px 10px; background:#4CAF50; color:white; border:none; border-radius:4px; cursor:pointer;">
                Check Location
            </button>
        </form>
    <?php elseif (!empty($order['tracking_number'])): ?>
        <!-- existing tracking logic remains here -->
        <a href="<?php echo $tracking_urls[$carrier]; ?>" class="tracking-link" target="_blank">Track Package</a>
    <?php else: ?>
        N/A
    <?php endif; ?>
</td>
